...
Tests of the prediction \(\frac{s^*}{g(K)} = 1\), uta, find \(\frac{s^*}{g(K)} = 0.481\) on average over all years and countries, GDP-weighted, from data for net saving and market-value capital in national accounts. Regression of \(s^*\) on \(g(K)\) shows as 0.1857 in Table \ref{tbl-4}. Fig. \ref{fig-si_plots} shows findings for \(\theta^*_s\) at 0.064 over the same period and countries. Table \ref{tbl-wid_si_table} shows regression of \(\Delta s^*\) on \(\Delta g(K)\) at 0.1000.
...
\begin{figure}[pos=H]
    ...
    \caption{\(\frac{s^*}{g(K)}\) 1980:2022 across countries, weighted to GDP} % and \(\theta^*_c\)
